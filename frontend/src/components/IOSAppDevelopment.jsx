...
export default function IOSAppDevelopment() {
  ...
  problems.`,
    },
    {
      ...
      heading: "iOS Application Design and \nStrategy",
  ...
  ensures the UI/UX matches iOS guidelines and delivers an impactful experience for end-users.`,
    },
    {
      ...
      heading: "iOS App Development and \nQuality Assurance",
      ...
      <div className="w-full h-full px-4 md:px-4 py-8 relative">
        ...
        <div className="absolute left-[-8%] top-1/2 transform -translate-y-1/2">
          <div className="flex flex-col items-center hidden sm:flex">
            <div
              className="text-[#529D92] font-Montserrat font-extrabold -rotate-89 whitespace-nowrap text-2xl md:text-[32px] tracking-wide origin-center"
              style={{
        ...
        <div className="ml-0 md:ml-24 h-full flex flex-col justify-center">
          ...
          <div className="w-full mb-10 overflow-x-auto scrollbar-hide">
            <div
              ...
              style={{ minWidth: `${steps.length * 280}px` }}
            >
              {steps.map((step, index) => (
                <div
                  key={index}
                  onClick={() => setActiveIndex(index)}
                  className={`app_step flex-shrink-0 flex items-center px-8.5 py-2 rounded-md border cursor-pointer ${
                    activeIndex === index
                      ? "bg-[#67C792] border-emerald-500 text-white"
                      : "bg-white border-gray-300 text-gray-800"
                  }`}
                >
                  <span className="mr-2 whitespace-nowrap text-[10px] md:text-[16px] font-semibold">
                    {step.title}
                  </span>
                  <MoveRight size={16} />
                </div>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="flex-1 max-w-[1000px]">
              <h1 className="text-3xl md:text-[48px] font-Montserrat font-extrabold text-black mb-4 leading-[62px] ">
                {steps[activeIndex].heading.split("\n").map((line, i, arr) => (
